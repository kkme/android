...
public class RefreshableListView extends ListView {
	...
	private void initialize() {
		...
		this.mListHeaderView
				.setOnHeaderViewOpendListener(new ListHeaderView.OnHeaderViewOpenedListener() {
					public void onHeaderViewOpened() {
						if (RefreshableListView.this.mListHeaderView
								.getChildCount() > 0) {
			...
			this.mListBottomView.startUpdate(new Runnable() {
				public void run() {
					...
					if (RefreshableListView.this.mOnPullUpUpdateTask != null) {
						RefreshableListView.this.mOnPullUpUpdateTask
								.updateBackground();
					}
					long delta = 500L - (System.currentTimeMillis() - b);
					if (delta > 0L) {
						try {
							Thread.sleep(delta);
					...
					RefreshableListView.this.post(new Runnable() {
						public void run() {
							int deltay = RefreshableListView.this.mListBottomView
									.close(0);
							RefreshableListView.this.postDelayed(
									new Runnable() {

										public void run() {

											if (RefreshableListView.this.mAutoNotifyAdapter != null) {
												RefreshableListView.this.mAutoNotifyAdapter
			...
			this.mListHeaderView.startUpdate(new Runnable() {
				public void run() {
					...
					if (RefreshableListView.this.mOnUpdateTask != null) {
						RefreshableListView.this.mOnUpdateTask
								.updateBackground();
					}
					long delta = 500L - (System.currentTimeMillis() - b);
					if (delta > 0L) {
		...
		case 2:
			if (this.mActivePointerId != -1) {
				...
				if (this.mState == 1) {
					...
					int activePointerIndex = MotionEventCompat
	...
	public static abstract interface OnBottomViewChangedListener extends
			RefreshableListView.OnHeaderViewChangedListener {
	}
	...
	public static abstract interface OnPullUpUpdateTask extends
			RefreshableListView.OnUpdateTask {
	}
