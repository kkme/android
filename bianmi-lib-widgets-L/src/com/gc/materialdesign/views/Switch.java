...
public class Switch extends CustomView {
	...
	boolean check = false;
	boolean press = false;
	private boolean mLastCheck = check;
	...
	protected void setAttributes(AttributeSet attrs) {
		...
		// Color by resource
		int bacgroundColor = attrs.getAttributeResourceValue(MATERIALDESIGNXML,
				"uncheckedColor", -1);
		if (bacgroundColor != -1) {
		...
		backgroundColor = attrs.getAttributeResourceValue(MATERIALDESIGNXML,
				"checkedColor", -1);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (event.getAction() == MotionEvent.ACTION_DOWN) {
			...
			// 跟上次不一样，才触发change事件
			if (mLastCheck != check) {
				mListener.onCheckedChanged(check);
				mLastCheck = check;
			}
		}
		return true;
	...
	protected void onDraw(Canvas canvas) {
		...
		temp.drawCircle(ball.getX() + ball.getWidth() / 2,
				ball.getY() + ball.getHeight() / 2, ball.getWidth() / 2,
				transparentPaint);
	...
	// ------------------------------Public------------------------------
	public OnSwitchChangeListener mListener = null;
